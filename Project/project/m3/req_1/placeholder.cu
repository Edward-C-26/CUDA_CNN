...
__global__ void float_to_half(half *out, const float *in, size_t height, size_t width, size_t rounded_height, size_t rounded_width) {
    ...
    if (col < rounded_width && row < rounded_height) {
        if (col < width && row < height) {
            ...
            out[col + row * rounded_width] = __float2half(0.0f);
        }
...
__global__ void wmma_matrix_mul(half *a, half *b, float *c, int M, int N, int K) {
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    dim3 grid(rounded_N / WMMA_M, rounded_M / WMMA_N, 1);
    dim3 block(32, 1, 1);  // One warp per block
    wmma_matrix_mul<<<grid, block>>>(weight_matrix_half, unrolled_matrix_half, out_padded_matrix, rounded_M, rounded_N, rounded_K);

    dim3 down_grid(rounded_N/16 * rounded_M/16 , 1);
    dim3 down_block(256, 1, 1);
    downsize_matrix<<<down_grid, down_block>>>(matmul_output, out_padded_matrix, Map_out, Width_unrolled, rounded_M, rounded_N);
